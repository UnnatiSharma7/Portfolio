...
const Skills = () => {
  return (
   ...
   <motion.h2
    className="mb-12 mt-20 text-center text-3xl font-semibold">Skills</motion.h2>
   <motion.div 
   initial="hidden"
   whileInView="visible"
   variants={containerVariants}
   viewport={{once:true}}
   className="mx-2 flex flex-col rounded-3xl px-4 py-2 lg:px-15
   border border-stone-50/30">
    {SKILLS.map((skill,index)=>(
        ...
        ${index !== SKILLS.length -1 ? "border-b border-stone-50/30": ""}`}>
            <div className="flex items-center px-1 py-1">{skill.icon}
